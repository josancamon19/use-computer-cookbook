...
def screenshot_cap_for_model(model: str) -> int:
    ...
    Lives in the cookbook (not the SDK) because the registry is model-specific
    and grows as new families ship — adding a model shouldn't require an SDK
    release. Per-family caps are probed for click accuracy; lower caps trade
    detail for stable coordinates on models that drift past their internal limit.
    """
    m = (model or "").lower()
    ...
    if "kimi" in m or "fireworks" in m:
        return 896  # Kimi: y-coord accuracy degrades at >1024 (probed on tall iOS shots)
                    # TODO: probe Qwen 3.6 Plus separately — it inherits Kimi's cap by coincidence
...
def scale_screenshot_for_model(
    ...
    Per Anthropic's CUA docs (and observed for other CUA-style models), vision
    models return coordinates in resized-image space. The model also silently
    resizes anything past its internal cap, so we resize ourselves to a known
    target and keep the inverse scale to undo it at dispatch time.
